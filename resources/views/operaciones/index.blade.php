...
@section("content")
    ...
    <div class="flex justify-center flex-wrap bg-gray-200 p-4 mt-5">
        <div class="text-center">
            <h1 class="mb-5">{{__("Operaciones Bachoco") }}</h1>
    ...
    <table class="border-separate border-2 text-center border-gray-500 mt-3" style="width: 100%">
        <thead>
        <tr>
            <th class="px-4 py-2">{{ __("Nombre de la operación") }} </th>
        ...
        <tr>
            ...
            <td class="border px-4 py-2">
                <a href="{{ route("operaciones.edit", [ "operacione" =>  $operacione ]) }}"  class="text-blue-400"> {{ __("Editar") }} </a>
                <a 
                    href="#"  
                    class="text-red-400"
                ...
                <form id="delete-operacione-{{ $operacione->id }}-form" action="{{ route("operaciones.destroy", ["operacione" => $operacione]) }}" method="POST" class="hidden">
                    @method("DELETE")
                    @csrf
                </form>
            </td>
